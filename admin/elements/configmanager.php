...
class JFormFieldConfigManager extends JFormField {
	...
	protected function getInput() {
			...
			if($task == 'load') {
				if(JFile::exists($base_path . $file)) {
					...
					if($result) {
						...
						$app->redirect($redirectUrl, JText::_('TPL_PISLICE_THEME_CONFIG_LOADED_AND_SAVED'), 'message');
					} else {
						// make an redirect
						$app = JFactory::getApplication();
						$app->redirect($redirectUrl, JText::_('TPL_PISLICE_THEME_CONFIG_SQL_ERROR'), 'error');
					}
				} else {
					// make an redirect
					$app = JFactory::getApplication();
					$app->redirect($redirectUrl, JText::_('TPL_PISLICE_THEME_CONFIG_SELECTED_FILE_DOESNT_EXIST'), 'error');
				}	
			} else if($task == 'save') {
				if($file == '') {
					$file = date('d_m_Y_h_s');
				...
				if(JFile::write($base_path . $file . (($i != 0) ? $i : '') . '.json' , $row->params)) {
					// make an redirect
					$app = JFactory::getApplication();
					$app->redirect($redirectUrl, JText::_('TPL_PISLICE_THEME_CONFIG_FILE_SAVED_AS'). ' '. $file . (($i == 0) ? '' : $i) .'.json', 'message');
				} else {
					// make an redirect
					$app = JFactory::getApplication();
					$app->redirect($redirectUrl, JText::_('TPL_PISLICE_THEME_CONFIG_FILE_WASNT_SAVED_PLEASE_CHECK_PERM'), 'error');
				}
			} else if($task == 'delete') {
				...
				if(JFile::exists($base_path . $file)) {
					...
					if($file == "default.config.json"){
						...
						if(JFile::delete($base_path . $file)) {						
							$app = JFactory::getApplication();
							$app->redirect($redirectUrl, $file . ' ' . JText::_('TPL_PISLICE_THEME_CONFIG_FILE_DELETED_AS'), 'message');
						} else {
							$app = JFactory::getApplication();
		...
		$html .= '<div><p><label>'.JText::_('TPL_PISLICE_THEME_LOAD_LABEL').'</label>'.$file_select.'<button id="config_manager_load" class="btn"><i class="icon-download"></i> '.JText::_('TPL_PISLICE_THEME_LOAD_BTN').'</button></p></div>';
		$html .= '<div class="input-append"><p><label>'.JText::_('TPL_PISLICE_THEME_SAVE_LABEL').'</label><input type="text" id="config_manager_save_filename" class="input-medium" placeholder="'.JText::_('TPL_PISLICE_THEME_SAVE_FILENAME').'" /><span class="add-on">.json</span><button id="config_manager_save" class="btn"><i class="icon-upload"></i> '.JText::_('TPL_PISLICE_THEME_SAVE_BTN').'</button></p></div>';
		$html .= '<div><p><label>'.JText::_('TPL_PISLICE_THEME_DELETE_LABEL').'</label>'.$file_delete.'<button id="config_manager_delete" class="btn"><i class="icon-remove"></i> '.JText::_('TPL_PISLICE_THEME_DELETE_BTN').'</button></p></div>';
		$html .= '<div><p><span class="label label-warning">'.JText::_('TPL_PISLICE_THEME_DIRECTORY').'</span> <span>'. $base_path .'</span></p></div>';
		$html .= '</div>';
		// finish the output
		return $html;
	}
